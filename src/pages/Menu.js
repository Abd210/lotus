...
export default function Menu() {
	...
	<div className="w-20 flex items-center justify-end space-x-2">
		{footerSettings.instagramUrl && (
			<a href={footerSettings.instagramUrl} target="_blank" rel="noopener noreferrer" className="text-gold active:opacity-70"><i className="fab fa-instagram text-lg"></i></a>
		)}
		{footerSettings.facebookUrl && (
			<a href={footerSettings.facebookUrl} target="_blank" rel="noopener noreferrer" className="text-gold active:opacity-70"><i className="fab fa-facebook text-lg"></i></a>
		)}
		{footerSettings.tiktokUrl && (
			<a href={footerSettings.tiktokUrl} target="_blank" rel="noopener noreferrer" className="text-gold active:opacity-70"><i className="fab fa-tiktok text-lg"></i></a>
		)}
	</div>
	</div>
				</div>
				...
				<div key={cat.id} className="cursor-pointer active:opacity-90" onClick={() => navigate(`/category/${cat.id}`)}>
					<div className="relative overflow-hidden rounded-xl border border-gold/30 shadow-lg">
						...
						<div className="absolute bottom-0 left-0 right-0 p-4">
							<h3 className="font-cinzel text-lg font-semibold text-off-white mb-2">{cat.name}</h3>
							{subs.length > 0 && (
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
					...
					<div className="hours-section">
						...
						<div className="space-y-2">
							<div className="flex justify-between"><span className="text-muted-gray">Monday–Thursday:</span><span className="text-off-white">{footerSettings.hoursMonThu}</span></div>
							<div className="flex justify-between"><span className="text-muted-gray">Friday–Saturday:</span><span className="text-off-white">{footerSettings.hoursFriSat}</span></div>
							<div className="flex justify-between"><span className="text-muted-gray">Sunday:</span><span className="text-off-white">{footerSettings.hoursSun}</span></div>
						</div>
					</div>
